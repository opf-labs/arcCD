...
/**
 * TODO Tests for CataloguedCdStack.
 * 
...
public final class CataloguedCdManager {
	...
	private CataloguedCdManager() {
		throw new AssertionError("Do not enter default constructor for "
				+ this.getClass().getName());
	}

	private CataloguedCdManager(final Map<Integer, CataloguedCd> cdsToManage,
			final Map<Integer, File> catalogueFiles) {
	...
	public CataloguedCd getCatalogueRecordById(final Integer requestedId) {
		if (this.managedCdRecords.containsKey(requestedId))
	 ...
	 * 
	 * @param rootDir
	 *            the root dir of the collection contain the info files
	 * @return and new manager instance
	 * @throws FileNotFoundException
	 */
	public static CataloguedCdManager fromDirectory(final File rootDir)
			throws FileNotFoundException {
		...
		File[] infoFiles = rootDir.listFiles(new FilenameFilter() {
			...
			public boolean accept(final File dir, final String name) {
				return FilenameUtils.isExtension(name.toLowerCase(),
						ArchiveCollection.INFO_EXT);
			}
		});
		for (File infoFile : infoFiles) {
			CdItemRecord item = CdItemRecord.fromInfoFile(infoFile);
			Integer intKey = Integer.valueOf(FilenameUtils.getBaseName(infoFile
					.getName()));
